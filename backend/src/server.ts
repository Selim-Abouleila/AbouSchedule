...
app.post('/test/admin-notifications', { preHandler: app.auth }, async (req, rep) => {
  try {
    ...
    console.log('🧪 [TEST] Manually triggering admin notification check...');
    
    // Import the function dynamically to avoid circular imports
    const { startAdminNotificationChecker } = await import('./lib/admin-notifications.js');
    
    // Create a test function that runs the notification logic once
    const testNotificationCheck = async () => {
      const now = new Date();
      console.log('🔔 [TEST] Checking for unread immediate tasks...');
      ...
      try {
        ...
        console.log(`📋 [TEST] Found ${unreadImmediateTasks.length} unread immediate tasks`);

        // Get all admin users' push tokens (moved outside the loop)
        ...
        const adminPushTokens = adminUsers.flatMap((user: any) => user.pushTokens);
        ...
        for (const task of unreadImmediateTasks) {
          ...
          if (adminPushTokens.length > 0) {
            ...
            console.log(`📤 [TEST] Sending notifications to ${adminPushTokens.length} admin devices...`);
            ...
            if (response.ok) {
              console.log(`✅ [TEST] Admin notifications sent successfully for task ${task.id}`);
              console.log(`📱 [TEST] Notified ${adminPushTokens.length} admin devices`);
              notificationsSent += adminPushTokens.length;
            } else {
              console.error(`❌ [TEST] Failed to send admin notifications for task ${task.id}:`, result);
            }
          } else {
        ...
        console.log(`🔔 [TEST] Check completed at ${now.toISOString()}`);
        return { 
          success: true, 
          message: 'Test completed',
          tasksChecked: unreadImmediateTasks.length,
          adminDevices: adminPushTokens.length,
          notificationsSent: notificationsSent
        };
      } catch (error) {
        console.error('❌ [TEST] Error in admin notification check:', error);
        throw error;
      }
    };
    ...
    return rep.code(500).send({ error: 'Test failed', details: error.message });
  }
});
